...
void common_attack(Player *p, EnemyType &e, int miss_combat_chance)
{
	...
	if (miss) // the player attacks the enemy and misses. do nothing.
		return;
	double atk = p->getAtk();
	double def = e.getDef();
...
std::string Drow::getRace() const { return "Drow"; }
...
std::string Vampire::getRace() const { return "Vampire"; }
...
std::string Troll::getRace() const { return "Troll"; }
...
std::string Goblin ::getRace() const { return "Goblin"; }
